...
fn main() -> io::Result<()> {
    ...
    if opt.decode && opt.encode {
        ...
        match result {
            Ok(_) => return Err(Error::new(ErrorKind::Other, "Cannot use --encode and --decode at the same time!")),
    ...
    while match buffer.last() { Some(b) => *b == b'\r' || *b == b'\n', None => false }
    {
    ...
    {
        match decode(buffer) {
            Ok(result) => return io::stdout().write_all(&result).and_then(|_| io::stdout().write_all(b"\n")),
            Err(e) => return Err(Error::new(ErrorKind::Other, e.to_string()))
        }
    }
    else
    {
        return io::stdout().write_all(&encode(buffer)).and_then(|_| io::stdout().write_all(b"\n"));
    }
}
